...
def latent_to_image(g_all, upsamplers, latents, return_upsampled_layers=False, use_style_latents=False,
    ...
    #print(f'Shape style_latents: {style_latents.shape}')
    ...
    for item in affine_layers:
        number_feature += item.shape[1]

    if return_upsampled_layers:
        affine_layers_upsamples = torch.FloatTensor(1, number_feature, dim, dim).cuda()
...
def prepare_model(args, classfier_checkpoint_path="", classifier_iter=10000, num_class=8, num_classifier=10, num_fold=0):
    print('----Prepare model----')
    print(f"Category: {args['category']}")
    if args['category'] == 'face' or args['category'] == 'flickr_car':
        res = 1024
        out_res = 512
    elif (args['category'] == 'face_256') or (args['category'] == 'car_modified') or (args['category'] == 'ISIC') or (args['category'] == 'clothes') or (args['category'] == 'cat') or (args['category'] == 'face_34'):
        res = 256
        out_res = 256
    ...
    if args['stylegan_ver'] == "1":
        ...
        #g_all = Stylegan2Generator(256, 32, 8, channel_multiplier=2, randomize_noise=False)
    ...
    if classfier_checkpoint_path != "":
        ...
        for MODEL_NUMBER in range(num_classifier):
            classifier = pixel_classifier(num_class, dim=args['dim'], factor=percentInputPerceptrons)
            ...
            if classifier_iter > 0:
                ...
                checkpoint = torch.load(os.path.join(classfier_checkpoint_path,
                                                     'model_' + str(MODEL_NUMBER) + '_fold_' + str(num_fold) + '.pth'))
