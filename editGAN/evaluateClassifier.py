...
#from utils.data_utils import clothes_palette as palette
#from utils.data_utils import ISIC_palette as palette
...
def run_seg(args, g_all, bi_upsamplers, classifier_list,  optimized_latent, index):
        ...
        if args['use_intermediate_layers']:
            intermediateLayer = torch.from_numpy(np.load(os.path.join( args['intermediate_layer'], 'intermediateLayers_000%d_0150.npy' % index))).to(device)
            ...
            print(f"intermediateLayer: {os.path.join( args['intermediate_layer'], 'intermediateLayers_000%d_0150.npy' % index)}")
...
def main(args):
    ...
    g_all, nn_upsamplers, bi_upsamplers, classifier_list, avg_latent = prepare_model(args,classfier_checkpoint_path=args['classfier_checkpoint'],classifier_iter=args['classifier_iter'], num_class=args['num_class'], num_classifier=args['num_classifier']) #classifier_iter=10000
    ...
    for i, id in enumerate(all_id):
        ...
        imageio.imsave(os.path.join(args['exp_dir'], f"evaluate_img_out_{id}.jpg"), img_out.astype(np.uint8))
        imageio.imsave(os.path.join(args['exp_dir'], f"evaluate_img_seg_final_{id}.png"), img_seg_final.astype(np.uint8))
