...
def embed_one_example(args, path, stylegan_encoder, g_all, upsamplers,
    ...
    imageio.imsave(os.path.join(sv_dir, "reconstruction.jpg"),curr_vis)

    imageio.imsave(os.path.join(sv_dir, "real_im.jpg"),raw_im_show[0])
...
def main(args, resume):
    ...
    stylegan_encoder = FPNEncoder(3, n_latent=n_latent, only_last_layer=args['use_w'], same_view_code=args['same_view_code'])
    stylegan_encoder = stylegan_encoder.to(device)
    ...
    for epoch in range(100):
        ...
        for i, da, in enumerate(train_data_loader):
            ...
            #if i % 500 == 0 and i!=0 and not args['debug']:
            #if i % 1000 == 0 and i!=0 and not args['debug']:
            if i % 2 == 0 and i!=0 and not args['debug']: #lsun
